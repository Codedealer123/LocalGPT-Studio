"""
llama_param_calculator.py

Given a model's weight size (GB) and quant type, computes a realistic,
hardware-aware set of constructor arguments for llama-cpp-python's Llama()
class — n_threads, n_ctx, n_batch, n_gpu_layers, use_mmap, use_mlock — and
explains the reasoning behind each choice.

CLI usage:
    python llama_param_calculator.py --weight-gb 4.37 --quant Q4_K_M
    python llama_param_calculator.py --params-b 7 --quant Q4_K_M
    python llama_param_calculator.py --weight-gb 4.37 --quant Q4_K_M --context 8192 --layers 32

Import usage:
    from llama_param_calculator import recommend, detect_host

    # quickest path — detects hardware itself
    result = recommend(weight_gb=4.37, quant="Q4_K_M")
    print(result.params.to_constructor_kwargs())

    # reuse a HostInfo across multiple models (avoids re-sampling RAM each time)
    host = detect_host(sample_seconds=15)
    result = recommend(weight_gb=4.37, quant="Q4_K_M", host=host, context=8192)

    from llama_cpp import Llama
    llm = Llama(**result.params.to_constructor_kwargs())

If --layers / total_layers is omitted, n_gpu_layers is estimated from total
model layers inferred from parameter count (rough heuristic) rather than
left as a guess with no basis.
"""
