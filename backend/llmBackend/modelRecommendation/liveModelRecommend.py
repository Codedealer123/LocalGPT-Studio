...
_COMFORT_HEADROOM_GB = {_J: 0.5, _H: 1.5, _K: 2.0}
_GPU_EFFICIENCY = 0.9
...
class ModelFamily:
    ...
    min_quant: str
    arch: ModelArch
MODEL_FAMILIES = [ModelFamily(0.5, '0.5B', 4, 'Nano / edge device', ['Qwen2.5-0.5B', 'SmolLM-360M'], _N, ModelArch(24, 2, 64, 896, 151936)), ModelFamily(1.0, '1B', 8, 'Tiny assistant, code completion', ['Llama-3.2-1B', 'Qwen2.5-1.5B'], _N, ModelArch(16, 8, 64, 2048, 128256)), ModelFamily(3.0, '3B', 8, 'Light chat / on-device assistant', ['Llama-3.2-3B', 'Phi-3.5-mini', 'Qwen2.5-3B'], _N, ModelArch(28, 8, 128, 3072, 128256)), ModelFamily(7.0, '7B', 8, 'General assistant (sweet-spot)', ['Mistral-7B-v0.3', 'Llama-3.1-8B', 'Qwen2.5-7B', 'Gemma-2-9B'], _M, ModelArch(32, 8, 128, 4096, 128256)), ModelFamily(14.0, '14B', 16, 'Strong reasoning & coding', ['Qwen2.5-14B', 'Phi-4-14B', 'Gemma-2-12B'], _M, ModelArch(48, 8, 128, 5120, 151936)), ModelFamily(22.0, '22B', 16, 'Near-GPT-3.5 quality', ['Qwen2.5-22B', 'Mistral-Small-22B'], _M, ModelArch(48, 8, 128, 6144, 32000)), ModelFamily(32.0, '32B', 32, 'Instruction-tuned powerhouse', ['Qwen2.5-32B', 'Mistral-Small-3.1-32B'], _L, ModelArch(64, 8, 128, 5120, 151936)), ModelFamily(47.0, '47B', 32, 'MoE efficiency (Mixtral-8x7B tier)', ['Mixtral-8x7B'], _L, ModelArch(32, 8, 128, 4096, 32000)), ModelFamily(70.0, '70B', 64, 'Frontier open-source quality', ['Llama-3.1-70B', 'Qwen2.5-72B', 'Nemotron-70B'], _L, ModelArch(80, 8, 128, 8192, 128256)), ModelFamily(123.0, '123B', 128, 'Very large; top open-weight quality', ['Mistral-Large-2', 'Mistral-Large-Instruct-2407'], 'Q2_K', ModelArch(88, 8, 128, 12288, 32768)), ModelFamily(236.0, '236B', 64, 'Huge MoE (Mixtral-8x22B tier)', ['Mixtral-8x22B'], _S, ModelArch(56, 8, 128, 6144, 32000)), ModelFamily(405.0, '405B', 128, 'Largest open-weight (Llama-3.1-405B)', ['Llama-3.1-405B'], 'IQ1_S', ModelArch(126, 8, 128, 16384, 128256))]
...
def _parse_nvidia_smi():
    try:
        D = subprocess.check_output([_P, '--query-gpu=index,name,memory.total,driver_version', '--format=csv,noheader,nounits'], text=_A, stderr=subprocess.DEVNULL)
        B = []
        ...
        try:
            F = subprocess.check_output([_P], text=_A, stderr=subprocess.DEVNULL)
...
def _parse_rocm():
    try:
        C = subprocess.check_output([_T, '--showmeminfo', 'vram', '--json'], text=_A, stderr=subprocess.DEVNULL)
        D = json.loads(C)
        A = []
        for E, B in D.items():
            if 'card' in E.lower():
...
class SystemInfo:
    ...
    @property
    def best_gpu(self):
        return max(self.gpus, key=lambda g: g.vram_gb) if self.gpus else _I
...
def detect_system():
    ...
    B = psutil.cpu_freq()
    J = round((B.max or B.current) / 1000, 2) if B else _B
    F = platform.processor() == 'arm' and platform.system() == _Q
    C = D = _F
    try:
...
class RamSample:
    duration_s: float
    samples_taken: int
    avg_available_gb: float
    min_available_gb: float
    max_available_gb: float
    stdev_gb: float
...
def sample_available_ram(seconds=DEFAULT_SAMPLE_SECONDS, quiet=_F):
    ...
    B = seconds
    B = max(10, min(30, B))
    D = max(2, int(round(B / SAMPLE_INTERVAL_SECONDS)))
    A = []
    E = time.monotonic()
    for H in range(D):
        F = psutil.virtual_memory().available / 1024 ** 3
        A.append(F)
        if not C:
            I = time.monotonic() - E
            print(f'\r   📊 Sampling RAM… {I:4.1f}s / {B}s (current available: {F:5.2f} GB)', end='', flush=_A)
        if H < D - 1:
    ...
    K = min(A)
    L = max(A)
    M = math.sqrt(sum(((A - G) ** 2 for A in A)) / len(A)) if len(A) > 1 else _B
    return RamSample(duration_s=round(J, 1), samples_taken=len(A), avg_available_gb=round(G, 2), min_available_gb=round(K, 2), max_available_gb=round(L, 2), stdev_gb=round(M, 2))
...
def compute_budget(sys, available_ram_gb=_I):
    ...
    if B.gpus and (not B.is_apple_silicon):
        A = B.total_vram_gb
        return MemoryBudget(source=_J, total_gb=round(A, 2), usable_gb=round(A * _GPU_EFFICIENCY, 2), note=B.gpu_names)
    if B.is_apple_silicon:
        A = B.ram_gb
        return MemoryBudget(source=_H, total_gb=round(A, 2), usable_gb=round(A * _APPLE_EFFICIENCY, 2), note='Apple Silicon – CPU and GPU share the same physical RAM pool')
    A = B.ram_gb
    return MemoryBudget(source=_K, total_gb=round(A, 2), usable_gb=round(A * _RAM_EFFICIENCY, 2), note='No GPU detected – llama.cpp will run in CPU-only mode')
...
class MemoryOverhead:
    ...
    def to_dict(A):
        B = _R if A.kv_dtype_bytes == 2 else 'int8'
        return {_U: round(A.kv_cache_gb, 3), 'kv_context_k': A.kv_context_k, _V: B, _W: round(A.cpu_compute_gb, 3), _X: round(A.linux_cache_gb, 3), 'tokenizer_gb': round(A.tokenizer_gb, 3), 'total_overhead_gb': A.total_gb}

def _measure_linux_cache_pressure_gb():
    try:
        A = psutil.virtual_memory()
        B = (getattr(A, 'cached', 0) + getattr(A, 'buffers', 0)) / 1024 ** 3
        return round(max(0.3, B * 0.3), 2)
    except Exception:
        return 0.5
...
def compute_overhead(model, context_k, kv_dtype_bytes, budget):
    ...
    G = E.source == _J
    H = E.source == _H
    J = platform.system() == 'Linux'
    K = 2 * A.num_layers * A.num_kv_heads * A.head_dim * F * D
    L = K / 1024 ** 3
    B = _B
    if not G and (not H):
        M = A.num_heads * LLAMA_N_BATCH * F * 4
        N = 3 * A.hidden_size * LLAMA_N_BATCH * 4
        B = max(0.1, (M + N) / 1024 ** 3)
        B = round(B, 3)
    I = _B
    if J and (not G) and (not H):
...
def build_recommendations(budget, sys_info, context_k, kv_dtype_bytes, min_bpw=_B):
    ...
    N = 4.0
    I = D.source == _K
    W = _COMFORT_HEADROOM_GB[D.source]
    O = []
    for C in QUANTS:
        if C.bpw < min_bpw:
            continue
        X = MemoryOverhead(kv_cache_gb=_B, kv_context_k=J, kv_dtype_bytes=K, cpu_compute_gb=_B, linux_cache_gb=_B, tokenizer_gb=_B)
        F = _I
        F = _I
        P = -1
        Q = X
        ...
        for E in MODEL_FAMILIES:
            G = compute_overhead(E, J, K, D)
            Z = E.params_b * C.factor + G.total_gb
            if Z > D.usable_gb:
                continue
            ...
            if I:
                ...
                B = 1.0
            S = E.params_b * 0.6 + C.bpw * 0.3 + B * 10
            if S > P:
                P = S
                F = E
                Q = G
                R = A
            if I and E.params_b > Y and (A < 3.0):
                continue
        if F is _I:
            continue
        T = F.params_b * C.factor
        G = Q
        A = R
        U = T + G.total_gb
        V = D.usable_gb - U
...
def top_picks(recs):
    ...
    for C in (_D, _E, _C, _G, _O):
        ...
        for D in [[A for A in A if A.fits_comfortably and A.is_practical], [A for A in A if A.fits_comfortably], [A for A in A if A.is_practical], A]:
            if D:
                B[C] = max(D, key=lambda r: (r.fits_comfortably, r.is_practical, r.est_tps, r.model.params_b, r.quant.bpw))
                break
    return B
...
def main():
    ...
    B.add_argument('--min-bpw', type=float, default=_B, help='Exclude quants below this BPW (e.g. 3.0 skips 1–2 bit quants)')
    ...
    E = detect_system()
    P = sample_available_ram()
    C = compute_budget(E, available_ram_gb=P.min_available_gb)
    ...
    print(F * 70)
    Q = f"  {'Tier':<14}  {'Quant':<12}  {'Model':<7}  {'Total GB':>8}  {'Headroom':>9}  {'~t/s':>6}"
    print(Q)
    print('  ' + F * 66)
    for R, D in I.items():
        S = '' if D.is_practical else ' 🐢'
        T = '✅' if D.fits_comfortably else '⚠️ '
        print(f'  {R:<14}  {D.quant.name:<12}  {D.model.label:<7}  {D.total_gb:>7.1f} GB  {D.headroom_gb:>6.1f} GB {T}  {D.est_tps:>5.0f}{S}')
    print(F * 70)
    J = A.write_md
