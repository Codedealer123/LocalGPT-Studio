...
Memory model covers all four costs that matter in practice:
  1. Transformer weights      quant_bpw × params / 8
...
DEFAULT_SAMPLE_SECONDS  = 15      # live RAM-sampling window (clamped to [10, 30])
SAMPLE_INTERVAL_SECONDS = 0.5      # poll interval while sampling
...
# ──────────────────────────────────────────────────────────────────────────────
# 3b.  LIVE RAM SAMPLING  (replaces single-snapshot estimate)
# ──────────────────────────────────────────────────────────────────────────────

@dataclass
class RamSample:
    duration_s:       float   # actual time spent sampling
    samples_taken:    int
    avg_available_gb: float   # mean of psutil "available" across the window — basis for budget
    min_available_gb: float   # worst point observed (most conservative reading)
    max_available_gb: float
    stdev_gb:         float   # spread; large stdev = bursty/unstable system


def sample_available_ram(seconds: int = DEFAULT_SAMPLE_SECONDS, quiet: bool = False) -> RamSample:
    """
    Poll psutil.virtual_memory().available repeatedly over a window and
    average the readings, instead of trusting a single instantaneous snapshot.

    A single sample can be misleading: a browser tab mid-GC, a backup job,
    or a one-off cron spike can make RAM look artificially free or full.
    Averaging over a real window (clamped to 10–30s) smooths that out and
    reflects sustained, not momentary, availability.

    Returned fields:
      avg_available_gb → used as the basis for the memory budget (typical conditions)
      min_available_gb → worst-case pressure observed during the window
      stdev_gb         → flags systems where memory is too volatile to trust a single number
    """
    seconds = max(10, min(30, seconds))
    n_polls = max(2, int(round(seconds / SAMPLE_INTERVAL_SECONDS)))

    readings_gb: list[float] = []
    start = time.monotonic()

    for i in range(n_polls):
        avail_gb = psutil.virtual_memory().available / (1024 ** 3)
        readings_gb.append(avail_gb)
        if not quiet:
            elapsed = time.monotonic() - start
            print(f"\r   📊 Sampling RAM… {elapsed:4.1f}s / {seconds}s "
                  f"(current available: {avail_gb:5.2f} GB)", end="", flush=True)
        if i < n_polls - 1:
    ...
    avg_gb = sum(readings_gb) / len(readings_gb)
    min_gb = min(readings_gb)
    max_gb = max(readings_gb)
    stdev_gb = (
        math.sqrt(sum((x - avg_gb) ** 2 for x in readings_gb) / len(readings_gb))
        if len(readings_gb) > 1 else 0.0
    )

    return RamSample(
        duration_s=round(actual_duration, 1),
        samples_taken=len(readings_gb),
        avg_available_gb=round(avg_gb, 2),
        min_available_gb=round(min_gb, 2),
        max_available_gb=round(max_gb, 2),
        stdev_gb=round(stdev_gb, 2),
    )
...
def compute_budget(
    ...
    if sys.gpus and not sys.is_apple_silicon:
        ...
        return MemoryBudget(
            ...
            usable_gb=round(total * _GPU_EFFICIENCY, 2),
            note=sys.gpu_names,
        )
    if sys.is_apple_silicon:
        ...
        return MemoryBudget(
            ...
            total_gb=round(total, 2),
            usable_gb=round(total * _APPLE_EFFICIENCY, 2),
    ...
    return MemoryBudget(
        ...
        total_gb=round(total, 2),
        usable_gb=round(total * _RAM_EFFICIENCY, 2),
...
def _measure_linux_cache_pressure_gb() -> float:
    """
    Read current page-cache pressure from psutil.
    We reserve ~30 % of current buffers+cached because:
      - dirty pages cannot be reclaimed under pressure
      - tmpfs, slab caches, and mmap'd executables are partially pinned
      - adding a 0.3 GB minimum for kernel structures (slab, page tables)
    """
    try:
        mem = psutil.virtual_memory()
        cached_gb = (getattr(mem, "cached", 0) + getattr(mem, "buffers", 0)) / (1024 ** 3)
        return round(max(0.3, cached_gb * 0.30), 2)
    except Exception:
        return 0.5
...
def compute_overhead(
    ...
    linux_cache_gb = 0.0
    if is_linux and not is_gpu and not is_apple:
...
def build_recommendations(
    ...
    for quant in QUANTS:
        ...
        best_model = None
        best_score = -1
        best_overhead = dummy_overhead
        best_tps : float = 1.0
        CPU_MAX_REASONABLE_PARAMS_B = 14.0
        ...
        for mf in MODEL_FAMILIES:
            ...
            tps = estimate_tps(mf.params_b * quant.factor, sys_info, budget)
        
            # --- CPU usability signal ---
            if is_cpu:
                if tps < CPU_UNUSABLE_TPS:
                    compute_penalty = 0.2
                elif tps < CPU_BAD_TPS:
                    compute_penalty = 0.6
                elif tps < CPU_OK_TPS:
                    compute_penalty = 0.85
                else:
                    compute_penalty = 1.0
            else:
                compute_penalty = 1.0
        
            # --- YOUR SCORE FUNCTION (PUT IT HERE) ---
            score = (
                (mf.params_b * 0.6) +
                (quant.bpw * 0.3) +
                (compute_penalty * 10)
            ...
            if is_cpu and mf.params_b > CPU_MAX_REASONABLE_PARAMS_B and tps < 3.0:
                continue
        ...
        recs.append(Recommendation(
            ...
            fits_comfortably=headroom >= comfort,
...
def main() -> None:
    ...
    ram_sample = sample_available_ram()
    
    budget = compute_budget(
        sys_info,
        available_ram_gb=ram_sample.min_available_gb,
    )
