...
LLAMA_N_BATCH=512
_COMFORT_HEADROOM_GB={_G:.5,_F:1.5,_H:2.}
COMFORT_RATIO=.12
...
MODEL_FAMILIES=[ModelFamily(.5,'0.5B',4,'Nano / edge device',['Qwen2.5-0.5B','SmolLM-360M'],_L,ModelArch(24,2,64,896,151936)),ModelFamily(1.,'1B',8,'Tiny assistant, code completion',['Llama-3.2-1B','Qwen2.5-1.5B'],_L,ModelArch(16,8,64,2048,128256)),ModelFamily(3.,'3B',8,'Light chat / on-device assistant',['Llama-3.2-3B','Phi-3.5-mini','Qwen2.5-3B'],_L,ModelArch(28,8,128,3072,128256)),ModelFamily(7.,'7B',8,'General assistant (sweet-spot)',['Mistral-7B-v0.3','Llama-3.1-8B','Qwen2.5-7B','Gemma-2-9B'],_K,ModelArch(32,8,128,4096,128256)),ModelFamily(14.,'14B',16,'Strong reasoning & coding',['Qwen2.5-14B','Phi-4-14B','Gemma-2-12B'],_K,ModelArch(48,8,128,5120,151936)),ModelFamily(22.,'22B',16,'Near-GPT-3.5 quality',['Qwen2.5-22B','Mistral-Small-22B'],_K,ModelArch(48,8,128,6144,32000)),ModelFamily(32.,'32B',32,'Instruction-tuned powerhouse',['Qwen2.5-32B','Mistral-Small-3.1-32B'],_J,ModelArch(64,8,128,5120,151936)),ModelFamily(47.,'47B',32,'MoE efficiency (Mixtral-8x7B tier)',['Mixtral-8x7B'],_J,ModelArch(32,8,128,4096,32000)),ModelFamily(7e1,'70B',64,'Frontier open-source quality',['Llama-3.1-70B','Qwen2.5-72B','Nemotron-70B'],_J,ModelArch(80,8,128,8192,128256)),ModelFamily(123.,'123B',128,'Very large; top open-weight quality',['Mistral-Large-2','Mistral-Large-Instruct-2407'],'Q2_K',ModelArch(88,8,128,12288,32768)),ModelFamily(236.,'236B',64,'Huge MoE (Mixtral-8x22B tier)',['Mixtral-8x22B'],_R,ModelArch(56,8,128,6144,32000)),ModelFamily(405.,'405B',128,'Largest open-weight (Llama-3.1-405B)',['Llama-3.1-405B'],'IQ1_S',ModelArch(126,8,128,16384,128256))]
@dataclass
class GPUInfo:index:int;name:str;vram_gb:float;driver_version:str='';cuda_version:str=''
def _parse_nvidia_smi():
	try:
		D=subprocess.check_output([_N,'--query-gpu=index,name,memory.total,driver_version','--format=csv,noheader,nounits'],text=_A,stderr=subprocess.DEVNULL);B=[]
		...
		try:
			F=subprocess.check_output([_N],text=_A,stderr=subprocess.DEVNULL);C=re.search('CUDA Version:\\s*([\\d.]+)',F)
			if C:
...
def _parse_rocm():
	try:
		C=subprocess.check_output([_S,'--showmeminfo','vram','--json'],text=_A,stderr=subprocess.DEVNULL);D=json.loads(C);A=[]
		for(E,B)in D.items():
...
class SystemInfo:
	...
	@property
	def best_gpu(self):return max(self.gpus,key=lambda g:g.vram_gb)if self.gpus else None
	@property
	def gpu_names(self):return', '.join(A.name for A in self.gpus)if self.gpus else'None'
def detect_system():
	H=psutil.virtual_memory();I=round(H.total/1024**3,1);B=psutil.cpu_freq();J=round((B.max or B.current)/1000,2)if B else .0;F=platform.processor()=='arm'and platform.system()==_O;C=D=_I
	try:
		if platform.system()==_P:G=Path('/proc/cpuinfo').read_text();C=' avx2 'in G;D=' avx512f 'in G
		elif platform.system()==_O:E=subprocess.check_output(['sysctl','-a','machdep.cpu.leaf7_features'],text=_A,stderr=subprocess.DEVNULL);C='AVX2'in E;D='AVX512F'in E
		elif platform.system()=='Windows':E=subprocess.check_output(['wmic','cpu','get','Caption'],text=_A,stderr=subprocess.DEVNULL)
	except Exception:pass
	A=[]
	if shutil.which(_N):A=_parse_nvidia_smi()
	elif shutil.which(_S):A=_parse_rocm()
...
def compute_budget(sys):
	B=sys
	if B.gpus and not B.is_apple_silicon:A=B.total_vram_gb;return MemoryBudget(source=_G,total_gb=round(A,2),usable_gb=round(A*_GPU_EFFICIENCY,2)-BROWSER_OVERHEAD_GB,note=B.gpu_names)
	if B.is_apple_silicon:A=B.ram_gb;return MemoryBudget(source=_F,total_gb=round(A,2),usable_gb=round(A*_APPLE_EFFICIENCY,2)-BROWSER_OVERHEAD_GB,note='Apple Silicon – CPU and GPU share the same physical RAM pool')
	A=B.ram_gb;return MemoryBudget(source=_H,total_gb=round(A,2),usable_gb=round(A*_RAM_EFFICIENCY,2)-BROWSER_OVERHEAD_GB,note='No GPU detected – llama.cpp will run in CPU-only mode')
@dataclass
class MemoryOverhead:
	...
	def to_dict(A):B=_Q if A.kv_dtype_bytes==2 else'int8';return{_T:round(A.kv_cache_gb,3),'kv_context_k':A.kv_context_k,_U:B,_V:round(A.cpu_compute_gb,3),_W:round(A.linux_cache_gb,3),'tokenizer_gb':round(A.tokenizer_gb,3),'total_overhead_gb':A.total_gb}
def _measure_linux_cache_pressure_gb():
	try:A=psutil.virtual_memory();B=(getattr(A,'cached',0)+getattr(A,'buffers',0))/1024**3;C=B*.1;return round(max(.3,C),2)
	except Exception:return .5
def compute_overhead(model,context_k,kv_dtype_bytes,budget):
	E=budget;D=kv_dtype_bytes;C=context_k;A=model.arch;F=C*1024;G=E.source==_G;H=E.source==_F;N=platform.system()==_P;J=2*A.num_layers*A.num_kv_heads*A.head_dim*F*D;K=J/1024**3;B=.0
	if not G and not H:L=A.num_heads*LLAMA_N_BATCH*F*4;M=3*A.hidden_size*LLAMA_N_BATCH*4;B=max(.1,(L+M)/1024**3);B=round(B,3)
	I=.0
	if platform.system()==_P and not G and not H:I=_measure_linux_cache_pressure_gb()
...
def estimate_tps(weights_gb,sys_info,budget):
	...
	if D<=0:return .0
	if E.source==_G:B=A.best_gpu.name if A.best_gpu else'';C=_match_bw(B,_GPU_BW,default=4e2)
	elif E.source==_F:B=A.gpus[0].name if A.gpus else'';C=_match_bw(B,_APPLE_BW,default=68.)
...
def _speed_label(bpw):
	for(A,B)in _SPEED_TABLE:
		if bpw<=A:return B
	return'Slow'
def build_recommendations(budget,sys_info,context_k,kv_dtype_bytes,min_bpw=.0):
	H=kv_dtype_bytes;G=context_k;A=budget;M=A.source==_H;O=_COMFORT_HEADROOM_GB[A.source];I=[]
	for B in QUANTS:
		if B.bpw<min_bpw:continue
		C=None
		for E in MODEL_FAMILIES:
			D=compute_overhead(E,G,H,A);N=E.params_b*B.factor+D.total_gb
			if N<=A.usable_gb:C=E
		if C is None:continue
		F=C.params_b*B.factor;D=compute_overhead(C,G,H,A);J=F+D.total_gb;K=A.usable_gb-J;L=estimate_tps(F,sys_info,A);I.append(Recommendation(quant=B,model=C,weights_gb=round(F,2),overhead=D,fits=J<=A.usable_gb,fits_comfortably=K>=_COMFORT_HEADROOM_GB[A.source],headroom_gb=round(K,2),speed_label=_speed_label(B.bpw),quality_stars=_TIER_STAR.get(B.tier,3),est_tps=L,is_practical=L>=MIN_PRACTICAL_TPS if M else _A))
	return I
def top_picks(recs):
	B={}
	for C in(_C,_D,_B,_E,_M):
		...
		for D in[[A for A in A if A.fits_comfortably and A.is_practical],[A for A in A if A.fits_comfortably],[A for A in A if A.is_practical],A]:
			if D:B[C]=max(D,key=lambda r:(r.model.params_b,r.quant.bpw));break
	return B
...
def generate_markdown(sys_info,budget,recs,picks,context_k,kv_dtype_bytes):
	G=kv_dtype_bytes;E=budget;C=sys_info;H=[];A=H.append;I=E.source==_H;A('# 🤖 LLM Hardware Compatibility Report');A('');A('> Auto-generated by `model_recommender.py` — llama.cpp-aware memory model');A('');A('## 🖥️ System');A('');A('| Property | Value |');A('|----------|-------|');A(f"| OS | `{C.os}` |");A(f"| CPU | `{C.cpu}` · {C.cpu_cores} physical cores @ {C.cpu_freq_ghz} GHz |");A(f"| RAM | **{C.ram_gb} GB** |");A(f"| AVX2 / AVX-512 | {C.has_avx2} / {C.has_avx512} |")
	if C.gpus:
	...
	elif E.source==_F:F=['Use `--n-gpu-layers 999` to offload all layers to Metal for maximum speed.','Add `--flash-attn` — Metal flash-attention is well-optimised on M-series.','Avoid `--no-mmap`; mmap is efficient on macOS due to unified memory.']
